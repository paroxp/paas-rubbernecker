...
internals.getStoriesByStatus = function(res, callback, status) {
    ...
    request(options, function getStories(error, response, body) {
        if (!error && response.statusCode == 200) {
            ...
            callback("Couldn't get stories due to error from Pivotal API: " + response.statusCode, null);
        }
    });
}
        ...
        calls.push(function (callback) {
            ...
            request(options, function getStories(error, response, body) {
                if (!error && response.statusCode == 200) {
                    ...
                    callback("Could not load the transitions for the story: " + story.id + " due to error from Pivotal API: " + response.statusCode, null);
                }
            });
        });
    });
...
internals.getSupportDetails = function (res, cb) {
    var endOfDay = new Date();
    endOfDay.setUTCHours(16, 00, 00, 00);

    var endOfYesterday = new Date();
    endOfYesterday.setDate(endOfDay.getDate() - 1);

    var options = {
        url: 'https://api.pagerduty.com/oncalls?time_zone=UTC&since=' + endOfYesterday.toISOString() + '&until=' + endOfDay.toISOString(),
        headers: {
            'Authorization': 'Token token=' + res.app.get('pagerdutyAPIToken'),
            'Accept': 'application/vnd.pagerduty+json;version=2'
        }
    };

    request(options, function getSchedules(error, response, body) {
        if (!error && response.statusCode == 200) {
            var list = {},
                obj = JSON.parse(body);
    ...
    async.parallel(
        {
            ...
            support: function (callback) {
                internals.getSupportDetails(res, callback);
            }
        },
